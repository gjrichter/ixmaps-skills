...
- In multi-layer maps (base `FEATURE` + thematic overlays), all related layers must use the same `ixmaps.layer("...")` name.
...
- For clean thematic backgrounds, use `mapType: "white"` and add a country boundary base layer (for example GISCO countries).
